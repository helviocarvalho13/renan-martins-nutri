...
async function POST(request) {
    ...
    const appointmentType = validTypes.includes(type) ? type : "FIRST_VISIT";
    ...
    let patientId = null;
    const { data: existingUser } = await supabase.from("profiles").select("id").ilike("id", `%`).limit(1);
    const { data: authUsers } = await supabase.auth.admin.listUsers();
    const matchingUser = authUsers?.users?.find((u)=>u.email?.toLowerCase() === patient_email.trim().toLowerCase());
    if (matchingUser) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__c02b47bf._.js.map