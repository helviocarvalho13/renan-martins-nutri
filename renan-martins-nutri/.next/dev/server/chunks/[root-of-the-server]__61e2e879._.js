...
async function GET() {
    ...
    const { data: activeReturns } = await supabase.from("appointments").select("id").eq("patient_id", user.id).eq("type", "RETURN").gte("date", today).in("status", [
        "PENDING",
        "CONFIRMED"
    ]).limit(1);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__61e2e879._.js.map