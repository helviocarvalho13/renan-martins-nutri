...
async function GET(request) {
    ...
    const { data: configs } = await supabase.from("schedule_config").select("*").eq("day_of_week", dayOfWeek).eq("is_active", true);
    ...
    const [apptRes, blockedRes] = await Promise.all([
        supabase.from("appointments").select("start_time").eq("date", date).in("status", [
            "PENDING",
            "CONFIRMED"
        ]),
...
//# sourceMappingURL=%5Broot-of-the-server%5D__c3e9c2ce._.js.map